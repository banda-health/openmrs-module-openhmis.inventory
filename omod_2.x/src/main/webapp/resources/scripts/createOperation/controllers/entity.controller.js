    ...
    function CreateOperationController($stateParams, $injector, $scope, $filter, EntityRestFactory, OperationModel,
        ...
        self.bindExtraVariablesToScope = self.bindExtraVariablesToScope
            || function(uuid) {
                ...
                $scope.changeDistributionType = self.changeDistributionType;

                $scope.returnOperationTypes = ["Department", "Institution", "Patient"];
                $scope.returnOperationType = $scope.returnOperationTypes[0];
                $scope.changeReturnOperationType = self.changeReturnOperationType;
        ...
        self.validateBeforeSaveOrUpdate = self.validateBeforeSaveOrUpdate || function() {
                ...
                var validateOpNum = CreateOperationFunctions.validateOperationNumber($scope);
                if(!validateOpNum){
                    return false;
                }
                ...
                if ($scope.destinationStockroom !== undefined && $scope.destinationEnabled){
                    $scope.entity.destination = $scope.destinationStockroom.uuid;
                }

                if ($scope.institutionStockroom !== undefined && $scope.institutionEnabled){
                    $scope.entity.institution = $scope.institutionStockroom.uuid;
                }

                if ($scope.department !== undefined && $scope.departmentEnabled){
                    $scope.entity.department = $scope.department.uuid;
                }

                // validate selected patient
                var validatePatient = CreateOperationFunctions.validatePatient($scope);
                if(!validatePatient){
                    return false;
                }

                // validate attribute types
                var validateAttributes = CreateOperationFunctions.validateAttributeTypes($scope);
                if(!validateAttributes){
                    return false;
                }

                // validate selected line items.
                var validateItems = CreateOperationFunctions.validateLineItems($scope);
                if(!validateItems){
                    return false;
                }
        ...
        self.changeDistributionType = self.changeDistributionType || function(){
                return CreateOperationFunctions.changeDistributionType($scope);
            }

        self.changeReturnOperationType = self.changeReturnOperationType || function(){
                return CreateOperationFunctions.changeReturnOperationType($scope);
            }

        self.changeOperationType = self.changeOperationType || function(){
                CreateOperationFunctions.changeOperationType($scope);
            }
                ...
                if($scope.lineItems.length > 0){
                    ...
                    if(lineItem.itemStock !== ""){
                        if(source === 'operationType'){
                            ...
                            CreateOperationFunctions.changeWarningDialog($scope, newVal, source);
